...
void ExtractInstanceDataFromGltfModel(
  ...
  // Extracting EXT_mesh_gpu_instancing data from a model converted by
  // I3dmToGltfConverter
       ...
       tilesetLayer = this->_tilesetGameObject.layer()](
        ...
        if (isI3dmType) {
          ...
          if (!instanceData.empty() && meshes.Length() != 0) {
            UnityEngine::Mesh baseMesh = unityMesh;
            ...
            if (currentI3dmIndex < 0) {
              ...
              // recent global index
              int32_t fallbackIndex = std::max(i3dmGlobalIndex - 1, 0);
              groupId += "#i3dm=" + std::to_string(fallbackIndex);
            } else {
              groupId += "#i3dm=" + std::to_string(currentI3dmIndex) +
            ...
            for (size_t i = 0; i < intanceObjects.size(); ++i) {
              ...
              UnityEngine::GameObject& instanceGameObject = intanceObjects[i];
