        ...
        if (isI3dmType) {
          ...
          for (size_t instanceDataIndex = 0;
            ...
            UnityEngine::GameObject intanceGameObject(System::String(
                "I3dm " + std::to_string(currentI3dmIndex) +
        ...
        if (isI3dmType) {
          ...
          if (!instanceData.empty() && meshes.Length() != 0) {
            ...
            if (currentI3dmIndex < 0) {
              // This should not normally happen, but fall back to the most recent global index
              int32_t fallbackIndex = std::max(i3dmGlobalIndex - 1, 0);
              groupId += "#i3dm=" + std::to_string(fallbackIndex);
            } else {
              groupId +=
                  "#i3dm=" + std::to_string(currentI3dmIndex) + "/primitive=" +
                  std::to_string(primitiveIndex);
            }
